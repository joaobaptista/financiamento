...
# ==============================================================================
# 14. Configuração do Mercado Pago (PIX)
# ==============================================================================

status "Configurando Mercado Pago (PIX)"

read -p "Deseja configurar o Mercado Pago para pagamentos PIX agora? (s/N): " CONFIG_MP

if [[ "$SEED_DB" =~ ^[Ss]$ ]]; then
    read -p "Digite seu MERCADOPAGO_ACCESS_TOKEN: " MP_ACCESS_TOKEN
    read -p "Digite sua VITE_MERCADOPAGO_PUBLIC_KEY: " MP_PUBLIC_KEY
    
    sed -i "s/^PAYMENTS_DRIVER=.*/PAYMENTS_DRIVER=mercadopago/" .env
    sed -i "s/^MERCADOPAGO_ACCESS_TOKEN=.*/MERCADOPAGO_ACCESS_TOKEN=$MP_ACCESS_TOKEN/" .env
    sed -i "s/^VITE_MERCADOPAGO_PUBLIC_KEY=.*/VITE_MERCADOPAGO_PUBLIC_KEY=$MP_PUBLIC_KEY/" .env
    
    info "Mercado Pago configurado no .env"
else
    info "Mercado Pago não configurado. O driver padrão será 'mock'."
fi

# ==============================================================================
# 15. Execução das Migrations e Seeding
# ==============================================================================
...
# ==============================================================================
# 16. Link Simbólico do Storage
# ==============================================================================

status "Criando link simbólico do storage (php artisan storage:link)"

sudo -u "$REAL_USER" php artisan storage:link

# ==============================================================================
# 17. Compilação de Assets
# ==============================================================================
...
# Garantir que o diretório de covers existe
mkdir -p "$PROJECT_DIR/storage/app/public/campaign-covers"

# Definir dono como o usuário real e grupo www-data para o servidor web
chown -R "$REAL_USER":www-data "$PROJECT_DIR"
chmod -R 775 "$PROJECT_DIR/storage"
chmod -R 775 "$PROJECT_DIR/bootstrap/cache"
chmod -R 775 "$PROJECT_DIR/storage/app/public"
...
echo ""
echo -e "${YELLOW}⚠️ IMPORTANTE PARA O PIX:${NC}"
echo -e "1. Certifique-se de que o ${BLUE}PAYMENTS_DRIVER=mercadopago${NC} no .env"
echo -e "2. Configure o Webhook no painel do Mercado Pago para:"
echo -e "   ${BLUE}http://$DOMAIN/api/webhooks/mercadopago${NC}"
echo -e "3. Para testes locais, use ${BLUE}ngrok${NC} para expor sua URL."
echo ""
